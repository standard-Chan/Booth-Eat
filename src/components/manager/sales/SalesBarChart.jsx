...
// today / yesterday 시리즈 병합
function mergeSeries(today = [], yesterday = []) {
  const byId = new Map();
  today.forEach((it) =>
    byId.set(it.menuItemId, { name: it.name, today: it.totalSales || 0, yesterday: 0 })
  );
  yesterday.forEach((it) => {
    const cur = byId.get(it.menuItemId) || { name: it.name, today: 0, yesterday: 0 };
    cur.yesterday = it.totalSales || 0;
...
// 라벨 너무 길면 말줄임
const ellipsis = (s = "", max = 6) => (s.length > max ? s.slice(0, max) + "…" : s);

export default function SalesBarChart({ itemsToday = [], itemsYesterday = [] }) {
  const data = useMemo(() => mergeSeries(itemsToday, itemsYesterday), [itemsToday, itemsYesterday]);
  ...
  const maxVal = useMemo(() => {
    const vals = data.flatMap((d) => [d.today, d.yesterday]);
    return Math.max(1, ...vals);
  }, [data]);

  const yScale = (v) => innerH - (v / maxVal) * innerH;

  const groupGap = 16; // 그룹 간격
  const groupW = data.length > 0 ? innerW / data.length : 0;
  const barW = Math.max(8, (groupW - groupGap) / 2);
    ...
    <Wrap ref={ref}>
      {tip && (
        <Tooltip style={{ left: tip.x, top: tip.y }}>
          ...
          <div>어제: {formatKRW(tip.yesterday)}</div>
      ...
      <svg width="100%" height="100%" viewBox={`0 0 ${size.w} ${size.h}`}>
        ...
        <g transform={`translate(${margin.left}, ${margin.top})`}>
          ...
          {data.map((d, idx) => {
            ...
            const yH = innerH - yScale(d.yesterday);
            const tH = innerH - yScale(d.today);

            const yesterdayX = gx;
            const todayX = gx + barW + 6; // 막대 사이 약간 간격
        ...
        <g transform={`translate(${margin.left}, ${margin.top - 4})`}>
          ...
          <text x={18} y={2} fontSize="12" fill="#6b7280">어제</text>
          <rect x={54} y={-8} width={12} height={12} rx={2} fill="#f97316" />
          <text x={72} y={2} fontSize="12" fill="#6b7280">오늘</text>
        </g>
      </svg>
    </Wrap>
  );
