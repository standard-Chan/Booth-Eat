...
- 주문 카드
  ...
  - 받아온 정보 카드에 렌더링
  - 주문 상태가 PENDING 인 경우
    - 주문 수락/거절 -> API 로 주문 수락/거절 데이터 전달
  - 주문 상태가 ACCEPTED 인 경우
    - 각각 메뉴에 대해서 체크를 통해 완료 여부 확인할 수 있도록 하기
      (중요한 정보이므로 서버에 저장시키면 더 좋음)
    - 비우기 버튼 생성 -> "alert 띄우기" -> API로 비워졌다는 정보 전달
  - 주문 수락된 상태인 경우 -> 비우기 버튼으로 변경
- 
...
- 팝업 닫기 기능
  ...
  - 팝업 외부 클릭시 팝업 닫힘
- 각 카드 별 (booth, 테이블번호에 해당하는 데이터 가져오기)
  - order 상태
    - 진행중인 경우 : 테이블 비우기 버튼 활성화
    - 완료된 경우 : 별도 처리 x
